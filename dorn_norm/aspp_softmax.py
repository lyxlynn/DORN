...
#    model.load_state_dict(saved_state_dict)
    ...
    for index, batch in enumerate(testloader):
        ...
        output = nyu_dataloader.get_depth_sid(torch.from_numpy(output).cuda())
...
def main():
   ...
   # criterion = CriterionDataParallel(criterion)
    ...
    ignore_params = conv1_params+conv2_params
   # print(ignore_params)
   # print(conv1_params.name)
   # print(conv2_params)
    base_params = filter(lambda p: id(p) not in ignore_params, deeplab.parameters())
    optimizer = optim.SGD([{'params': filter(lambda p: p.requires_grad, base_params), 'lr': args.learning_rate }], 
   ...
   # lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer,[10,15,18],0.1)          
    ...
    for i_iter, batch in enumerate(trainloader):
      ...
      #      lr_scheduler.step()
        ...
        loss = criterion(pred_ord, labels)
        loss.backward()
        optimizer.step()

        preds = nyu_dataloader.get_depth_sid(pred_d)
        
        labels = nyu_dataloader.get_depth_sid(labels.float())
        if i_iter % 100 == 0:
  ...
  #          torch.save(deeplab.state_dict(),osp.join(args.snapshot_dir, 'nyu_'+str(i_iter)+'.pth'))     
            ...
            if error[4]>0.75 or i_iter % 10000==0:
                torch.save(deeplab.state_dict(),osp.join(args.snapshot_dir, 'nyu_'+str(i_iter)+'.pth'))     
            model.train()
