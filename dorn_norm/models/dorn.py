...
class FullImageEncoder(nn.Module):
    def __init__(self):
        ...
        self.global_pooling = nn.AvgPool2d(8, stride=8)  # KITTI 16 16
        self.dropout = nn.Dropout2d(p=0.5)
        self.global_fc = nn.Linear(2048 * 3 * 5, 512)
        self.relu = nn.ReLU(inplace=True)
        self.conv1 = nn.Conv2d(512, 512, 1)  # 1x1 卷积
        self.upsample = nn.UpsamplingBilinear2d(size=(31, 41))  # KITTI 49X65 NYU 33X45
...
class Classifier_Module(nn.Module):
    ...
    def __init__(self,dilation_series,padding_series,NoLabels):
        ...
        for dilation,padding in zip(dilation_series,padding_series):
            self.aspp = nn.Sequential(nn.Conv2d(2048, 512, 3, padding= padding, dilation =dilation, bias = True),
                                    nn.ReLU(inplace=True),
                                    nn.Conv2d(512, 512, 1),
                                    nn.ReLU(inplace=True))
            self.conv2d_list.append(self.aspp)
        ...
        self.concat_process = nn.Sequential(
            ...
            nn.Upsample(scale_factor =8,mode='bilinear'))  # KITTI 142 NYU 136 In paper, K = 80 is best, so use 160 is good!
    ...
    def forward(self, x):
        ...
        for i in range(len(self.conv2d_list)):
            out = torch.cat((out, self.conv2d_list[i](x)), dim =1)
        out = self.concat_process(out) 
        return out
...
class ResNet(nn.Module):
    def __init__(self, block, layers,NoLabels):
        ...
        self.layer5 = self._make_pred_layer(Classifier_Module, [4,8,12],[4,8,12],NoLabels)
