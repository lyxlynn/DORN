...
class LIPParsingEdgeDataSet(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, crop_size=(473, 473), mean=(128, 128, 128), scale=False, mirror=True, ignore_label=255):
        ...
        for item in self.img_ids:
           # image_path, label_path, label_rev_path, edge_path = item
            ...
            norm_path = label_path.replace('dep.mat','norm.npy')
            name = osp.splitext(osp.basename(label_path))[0]  
            img_file = osp.join(self.root, image_path)
            label_file = osp.join(self.root, label_path) 
            norm_file = osp.join(self.root, norm_path) 
            self.files.append({
                "img": img_file,
                "label": label_file,
                "norm": norm_file,
    ...
    def __getitem__(self, index):
        ...
        if self.is_mirror:
            ...
            label = label[:, ::flip]
            norm = norm[:,::flip,:]
            norm[:,:,1]=norm[:,:,1]*(-1) ## shift: multiply the corresponding x-direction with -1
        #if np.max(label) > 20:
        ...
        ###label
        return image.copy(), self.get_depth_sid(label),norm.copy(), np.array(size), name  
