...
class LIPParsingEdgeDataSet(data.Dataset):
    ...
    def data_augmentation(self, im_rgb, im_d, out_h, out_w):
        ...
        def random_scaling(im, im_d):
            ...
            im = cv2.resize(im,None,fx = scaling, fy = scaling,interpolation = cv2.INTER_LINEAR)
            out_h,out_w ,_ = im.shape
            im_d = cv2.resize(im_d, (out_h, out_w))/scaling
            return im, im_d
        ...
        def random_cropping(im, im_d, out_h, out_w):
            ...
            img_h, img_w = im_d.shape
            h_off = random.randint(0, img_h - out_h)
            w_off = random.randint(0, img_w - out_w) 
            image = im[h_off : h_off+self.crop_h, w_off : w_off+self.crop_w]
            label = im_d[h_off : h_off+self.crop_h, w_off : w_off+self.crop_w]
    ...
    def __getitem__(self, index):
        ...
        image = cv2.resize(image,(240,320), interpolation = cv2.INTER_LINEAR)
        
        label = cv2.resize(label,(240,320))
    #    image,label = self.data_augmentation(image, label, 228,304 )
        print(self.is_mirror,'miroooooo')
        if self.is_mirror:
