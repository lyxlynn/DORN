...
#from mydataset import LIPParsingEdgeDataSet
...
def main():
   ...
   #     print('labels174',labels)
        images = Variable(images.cuda())
        labels = Variable(labels.cuda())
        #scipy.misc.imsave('{}.png'.format(i_iter),labels[0])
        optimizer.zero_grad()
        lr = adjust_learning_rate(optimizer, i_iter)
        preds = model(images)
        loss = criterion(preds, labels)
       # acc = pixel_acc(preds[1],labels)
        loss.backward()
        optimizer.step()
        ...
        if i_iter % (args.save_pred_every*100) == 0:
