...
flags.DEFINE_float("reg_rate",                    1e-5,    "reg rate")
...
flags.DEFINE_float("drop_out",                     0.5,    "drop out")
...
class FCRN(object):
                ...
                with tf.variable_scope(op_name, reuse=True):
                    for param_name, data in data_dict[op_name].iteritems():
                        try:
                            ...
                            var = tf.get_variable(param_name)
                            sess.run(var.assign(data))
                            # if op_name == 'bn_conv1' and param_name == 'scale':
                            #     print('--------------------------')
                            #     sess.run(var.assign(data*0.05))
                        except:
    ...
    def build_inference(self):
        ...
        with tf.variable_scope('dp', reuse=True):
            ...
            elif opt.net_type == 'ResNet50UpProj_my':
                net = models.ResNet50UpProj_my({'data': rgb_input}, self.batch_size, 1, False)
    ...
    def train(self):
        ...
        print('init_weight_file',opt.init_weight_file)
        ...
        with tf.variable_scope('dp', reuse=tf.AUTO_REUSE, regularizer=tf.contrib.layers.l2_regularizer(opt.reg_rate)):
            if opt.net_type == 'ResNet50UpProj':
                if opt.fixbn == 'false':
                    net = models.ResNet50UpProj({'data': rgb_input}, self.batch_size, opt.drop_out, True)
                elif opt.fixbn == 'true':
                    net = models.ResNet50UpProj({'data': rgb_input}, self.batch_size, opt.drop_out, False)
            elif opt.net_type == 'ResNet50UpProj_aspp':
                if opt.fixbn == 'false':
                    net = models.ResNet50UpProj_aspp({'data': rgb_input}, self.batch_size, opt.drop_out, True)
                elif opt.fixbn == 'true':
                    net = models.ResNet50UpProj_aspp({'data': rgb_input}, self.batch_size, opt.drop_out, False)
            elif opt.net_type == 'ResNet50UpProj_my':
                if opt.fixbn == 'false':
                    net = models.ResNet50UpProj_my({'data': rgb_input}, self.batch_size, opt.drop_out, True)
                elif opt.fixbn == 'true':
                    net = models.ResNet50UpProj_my({'data': rgb_input}, self.batch_size, opt.drop_out, False)
        ...
        reg_loss_smry = tf.summary.scalar("reg_loss", reg_loss)
        ...
        learning_rate = tf.train.exponential_decay(self.learning_rate, global_step, 10000, 0.9, staircase=True)
        # boundaries = [600000, 1200000]
        # values = [1.0, 0.5, 0.1]
        ...
        self.merged_summary_train = tf.summary.merge([pred_min_smry, pred_max_smry, pred_mean_smry, gt_min_smry,gt_max_smry, gt_mean_smry, 
                                                        pixel_loss_smry, ssim_loss_smry, tot_loss_smry,reg_loss_smry, rgb_input_smry, gt_resize_smry, mask_smry, pred_depth_smry, lrrate_smry])
        ...
        with tf.Session() as sess:
            ...
            # ========================================
            # with tf.variable_scope("dp", reuse=True):
            #     print(sess.run(tf.get_variable('bn_conv1/scale')))
    ...
    def data_augmentation(self, im_rgb, im_d, out_h, out_w):
        ...
        def random_coloring_2(im):
            batch_size, in_h, in_w, in_c = im.get_shape().as_list()
            im_aug = tf.image.convert_image_dtype(im, tf.float32)
